...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.jadwalSale.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.jadwal-sales.update", [$jadwalSale->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name_partner">{{ trans('cruds.jadwalSale.fields.name_partner') }}</label>
                <input class="form-control {{ $errors->has('name_partner') ? 'is-invalid' : '' }}" type="text" name="name_partner" id="name_partner" value="{{ old('name_partner', $jadwalSale->name_partner) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name_end_user') ? 'is-invalid' : '' }}" type="text" name="name_end_user" id="name_end_user" value="{{ old('name_end_user', $jadwalSale->name_end_user) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text" name="date" id="date" value="{{ old('date', $jadwalSale->date) }}" required>
                @if($errors->has('date'))
                    <div class="invalid-feedback">
                        {{ $errors->first('date') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('time') ? 'is-invalid' : '' }}" type="text" name="time" id="time" value="{{ old('time', $jadwalSale->time) }}" required>
                @if($errors->has('time'))
                    <div class="invalid-feedback">
                        {{ $errors->first('time') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('agenda') ? 'is-invalid' : '' }}" type="text" name="agenda" id="agenda" value="{{ old('agenda', $jadwalSale->agenda) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('province') ? 'is-invalid' : '' }}" name="province" id="province" required>
                    ...
                    @foreach(App\Models\JadwalSale::PROVINCE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('province', $jadwalSale->province) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('address') ? 'is-invalid' : '' }}" type="text" name="address" id="address" value="{{ old('address', $jadwalSale->address) }}" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('meeting_result') ? 'is-invalid' : '' }}" name="meeting_result" id="meeting_result">{!! old('meeting_result', $jadwalSale->meeting_result) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('presales_requirement') ? 'is-invalid' : '' }}" name="presales_requirement" id="presales_requirement">
                    ...
                    @foreach(App\Models\JadwalSale::PRESALES_REQUIREMENT_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('presales_requirement', $jadwalSale->presales_requirement) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
